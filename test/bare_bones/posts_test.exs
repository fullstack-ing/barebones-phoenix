defmodule BareBones.PostsTest do
  use BareBones.DataCase
  ...
  describe "posts" do
    ...
    import BareBones.AccountsFixtures, only: [user_scope_fixture: 0]
    import BareBones.PostsFixtures

    @invalid_attrs %{free: nil, description: nil, title: nil, body: nil, slug: nil, published_at: nil, draft: nil}
    ...
    test "create_post/2 with valid data creates a post" do
      valid_attrs = %{free: true, description: "some description", title: "some title", body: "some body", slug: "some slug", published_at: ~D[2025-06-17], draft: true}
      scope = user_scope_fixture()

      assert {:ok, %Post{} = post} = Posts.create_post(scope, valid_attrs)
      assert post.free == true
      assert post.description == "some description"
      assert post.title == "some title"
      assert post.body == "some body"
      assert post.slug == "some slug"
      assert post.published_at == ~D[2025-06-17]
    ...
    test "update_post/3 with valid data updates the post" do
      scope = user_scope_fixture()
      post = post_fixture(scope)
      update_attrs = %{free: false, description: "some updated description", title: "some updated title", body: "some updated body", slug: "some updated slug", published_at: ~D[2025-06-18], draft: false}

      assert {:ok, %Post{} = post} = Posts.update_post(scope, post, update_attrs)
      assert post.free == false
      assert post.description == "some updated description"
      assert post.title == "some updated title"
      assert post.body == "some updated body"
      assert post.slug == "some updated slug"
      assert post.published_at == ~D[2025-06-18]
